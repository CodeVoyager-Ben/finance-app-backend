...
def _fetch_proxy_list():
    ...
    sources = [(url, 5) for url in GITHUB_PROXY_SOURCES] + [(url, 5) for url in PROXY_API_URLS]
    start = time.time()

    for url, per_timeout in sources:
        remaining = 8 - (time.time() - start)
        if remaining <= 0:
            break
        try:
            resp = requests.get(url, timeout=min(per_timeout, remaining))
...
def _find_working_proxy(proxy_list, max_test=20):
    """并行校验代理列表，返回第一个可用的（总超时 10 秒）"""
...
def get_proxy():
    ...
    now = time.time()
    with _cache_lock:
        cached = _proxy_cache['proxy']
        expires = _proxy_cache['expires']

    if cached and now < expires:
        if _test_proxy(cached):
            return cached
        with _cache_lock:
            if _proxy_cache['proxy'] == cached:
