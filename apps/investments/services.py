...
def update_holding_from_transaction(transaction):
    ...
    holding, created = InvestmentHolding.objects.select_for_update().get_or_create(
    ...
    if transaction.transaction_type == 'buy':
        ...
        if transaction.quantity > holding.quantity:
            ...
            raise ValidationError(
                f'卖出数量({transaction.quantity})超过持仓数量({holding.quantity})'
            )
        ...
        if transaction.quantity <= 0:
            from rest_framework.exceptions import ValidationError
            raise ValidationError('拆股比例必须大于0')
