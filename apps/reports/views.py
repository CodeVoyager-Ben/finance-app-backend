...
class BalanceSheetView(APIView):
    ...
    def get(self, request):
        ...
        # 批量查询目标日期之后的交易净变动（避免 N+1）
        future_tx_by_account = {}
        if target_date < date.today():
            future_txs = Transaction.objects.filter(
                account__in=accounts, date__gt=target_date
            ).values('account').annotate(
                income=Sum('amount', filter=Q(transaction_type='income')),
                expense=Sum('amount', filter=Q(transaction_type='expense')),
            )
            for ft in future_txs:
                future_tx_by_account[ft['account']] = (ft['income'] or 0) - (ft['expense'] or 0)

        for acc in accounts:
            balance = acc.balance
            if target_date < date.today():
                net_change = future_tx_by_account.get(acc.id, 0)
