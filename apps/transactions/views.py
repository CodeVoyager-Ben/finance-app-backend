...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def _update_account_balance(self, transaction):
        ...
        if transaction.transaction_type == 'income':
            ...
            if transaction.to_account_id:
                to_account = Account.objects.select_for_update().get(pk=transaction.to_account_id)
                to_account.balance = F('balance') + transaction.amount
    ...
    def dashboard(self, request):
        ...
        # 账户总余额
        total_balance = request.user.accounts.filter(
            is_active=True, exclude_from_reports=False
