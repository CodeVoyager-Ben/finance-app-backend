...
| 收支记账 | `/transactions` | 四种类型（支出/收入/转账/还信用卡）、数字键盘快速记账、分类选择、多条件筛选（账户/分类/日期/搜索）、筛选汇总统计 |
| 日历视图 | `/calendar` | 按日查看收支、月度汇总、点击日期查看交易明细 |
| 投资管理 | `/investments` | 多资产类型、多币种、持仓盈亏、分红利息、资产配置、年化收益、日历监测每日盈亏 |
...
| 金额隐私隐藏 | 仪表盘/投资/借贷/报表/设置页面支持一键隐藏金额，眼睛按钮切换，状态独立持久化 |
...
6. **日历 Tab：**
   - 月份导航栏（本月盈亏、盈利天数、亏损天数）
   - "同步今日数据"按钮（手动触发行情获取并生成当日快照）
   - 日历网格：每日显示盈亏金额和百分比，红涨绿跌，背景色深浅随金额变化
   - 日期详情弹窗：点击某天展示该日所有持仓快照（收盘价/日盈亏/日盈亏%/总盈亏/总盈亏%）
...
回补缺失日期的持仓快照数据（通过东方财富历史K线API获取历史收盘价）。

```bash
python manage.py backfill_snapshots --start-date 2026-04-27 --end-date 2026-04-30  # 回补指定日期范围
python manage.py backfill_snapshots --start-date 2026-04-27 --end-date 2026-04-30 --dry-run  # 仅预览
python manage.py backfill_snapshots --start-date 2026-04-27 --end-date 2026-04-30 --user-id 1  # 指定用户
```

**说明：** 自动跳过已有快照的日期和非交易日（无K线数据的日期），使用 `update_or_create` 保证幂等。

---

## 关键行为说明

1. **账户余额维护：** 在 TransactionViewSet 的 `_update_account_balance()` 中管理，创建时更新，修改时先回滚旧值再应用新值，删除时回滚。所有余额变更操作使用 `db_transaction.atomic()` 事务保护，防止并发竞态导致余额不一致。
2. **账户余额字段保护：** `AccountSerializer` 在编辑账户时将 `balance` 设为只读，仅创建账户时允许设置初始余额。前端编辑弹窗也隐藏余额字段，防止编辑账户时误覆盖交易系统计算的正确余额。
3. **投资持仓自动更新：** 创建投资交易时，买入自动建仓或更新均价，卖出自动减仓，分红降低成本基准，再投资自动买入，拆股按比例调整。在 `services.py` 的 `update_holding_from_transaction()` 中处理。
4. **多币种自动换算：** 投资账户支持独立币种，通过 `ExchangeRate` 表手动录入汇率，`services.py` 的 `to_cny()` 自动换算为人民币汇总。持仓继承账户币种，也支持独立设置。
5. **分红自动处理：** 创建 `DividendRecord` 时自动生成关联 `InvestmentTransaction`，现金分红增加账户余额并降低成本，再投资自动生成买入交易，在 `services.py` 的 `handle_dividend()` 中处理。
6. **批量价格更新：** `batch_update_prices` 接口先将所有持仓的 current_price 复制到 previous_close_price，再设置新价格，确保日盈亏计算正确。
7. **借贷状态自动流转：** 录入还款时自动累加已还金额，全额还清时状态从 outstanding → settled。在 RepaymentViewSet 的 `perform_create` 中处理。
8. **注册自动初始化：** 新用户注册时调用 `init_default_data()` 创建全部默认分类和账户。
9. **报表支持历史数据：** 资产负债表接口可传 `date` 参数查看历史某天的数据，通过回算未来交易实现。投资持仓支持多币种，报表自动换算为人民币。
10. **全局数据隔离：** 所有 ViewSet 的 `get_queryset()` 都按 `request.user` 过滤，非管理员只能看到自己的数据。
11. **前端API代理：** Vite 开发服务器将 `/api` 请求代理到 Django 后端（`127.0.0.1:8000`），生产环境需配置 Nginx 反向代理。
12. **资产类型管理：** 9 个系统预设类型（user=null，不可修改删除），用户可自建扩展。
13. **自动行情获取（直连优先）：** 通过东方财富 kline API（`push2his.eastmoney.com`）获取A股最新价格和昨收价。采用**直连优先+代理备用**策略：先尝试直连东方财富API，成功则直接返回；直连失败时才获取代理IP重试。代理池总超时保护（获取8秒、验证10秒），避免代理不可用时长时间阻塞。
14. **手续费自动计算：** 买入/卖出交易自动按券商标准费率计算手续费：佣金万2.5（最低5元）、印花税0.05%（仅卖出）、过户费0.001%（双向）。代码在 `fee_calculator.py`。
...
22. **持仓唯一约束：** `InvestmentHolding` 模型有 `(investment_account, symbol)` 唯一约束，防止并发买入创建重复持仓。
23. **投资交易事务保护：** 投资交易创建（`InvestmentTransactionViewSet.create`）使用 `db_transaction.atomic()` 包裹，确保交易记录和持仓更新的一致性。`services.py` 中所有余额变更使用 `F()` 表达式 + `select_for_update()` 防止并发竞态。
24. **分红处理事务保护：** `handle_dividend()` 使用 `db_transaction.atomic()` 包裹，确保分红交易、持仓更新、账户余额变更的原子性。
25. **资产类型修改保护：** `AssetTypeViewSet` 对系统预设类型（user=null）的修改和删除使用 `PermissionDenied` 异常而非 `return Response`，确保 HTTP 状态码正确返回给客户端。
26. **还款事务保护与重算：** 还款的创建/编辑/删除均使用 `db_transaction.atomic()` + `select_for_update()` 保护。编辑或删除还款时自动重新计算父 `LendingRecord` 的 `repaid_amount`、`interest_amount` 和 `status`。
27. **还款金额校验：** 还款时校验金额不超过剩余欠款，使用 `Decimal` 精确计算（不使用 `float`）。
28. **借贷记录编辑限制：** 已有还款记录的借贷记录不允许修改 `amount` 和 `record_type` 字段，防止数据不一致。
29. **Token自动刷新：** 前端 401 响应拦截器自动使用 refresh_token 刷新 access_token，支持并发请求排队等待刷新完成，避免用户在 token 过期时被强制登出。
30. **投资日历自动刷新：** 自动更新价格后通过 `refreshKey` 触发日历组件重新加载数据，确保日历 Tab 实时反映最新快照。
31. **金额隐私隐藏：** 仪表盘、投资管理、借贷管理、报表中心、个人设置 5 个页面支持眼睛按钮一键隐藏所有金额数据。基于 Zustand 全局状态管理，每个页面状态独立（per-page），通过 `localStorage` 持久化。隐藏时金额统一显示为 `****`，图表 tooltip 同样隐藏。收支记账和日历视图不包含此功能。
